...
//       // const productQ = `/reports/returns/by-product?${dateQuery}${godownPart}${fq.replace(/^&/, '')}&metric=missing`
// //  const productQ =
// //   `/reports/returns/by-product?${dateQuery}godownId=${encodeURIComponent(godownId)}&billerUserId=${encodeURIComponent(billerUserId)}&metric=missing`
...
// const productQ =
//   `/reports/returns/by-product?date=${date}&godownId=6a06a76afb0121a70a2c12bc&billerUserId=${encodeURIComponent(billerUserId)}&metric=missing`
...
//                   <select
//                     value={billerUserId}
//                     onChange={(e) => setFilters({ billerUserId: e.target.value })}
//                     style={{
//                       height: 38, minWidth: 220, borderRadius: 8, border: '1px solid #e2e8f0',
//                       background: '#fff', padding: '0 12px', fontSize: 13, color: '#0f172a',
//                       boxShadow: '0 1px 2px rgba(0,0,0,0.04)', cursor: 'pointer', outline: 'none',
//                     }}
//                   >
//                     <option value="">— Select a biller —</option>
//                     {billers.map((b) => (
//                       <option key={b.id} value={b.id}>
//                         {b.name}{b.siteName ? ` — ${b.siteName}` : ''}
//                       </option>
//                     ))}
//                   </select>
//                   {billerUserId && (
...
function InlineSearchableSelect({
  ...
  React.useEffect(() => {
    ...
    const reposition = () => {
      if (btnRef.current) {
        ...
        const clampedLeft = Math.min(r.left, Math.max(8, window.innerWidth - DROPDOWN_W - 8))
        const spaceBelow = window.innerHeight - r.bottom
        const top = spaceBelow < DROPDOWN_H && r.top > DROPDOWN_H
          ? r.top - DROPDOWN_H - 4
          : r.bottom + 4
        setDropPos({ top, left: clampedLeft, width: r.width })
      }
    }
    window.addEventListener('scroll', reposition, true)
    window.addEventListener('resize', reposition)
  ...
  const handleToggle = () => {
    if (!open && btnRef.current) {
      const r = btnRef.current.getBoundingClientRect()
      const DROPDOWN_W = 360
      const DROPDOWN_H = 290 // approx max height (search box + list)
      // clamp left so dropdown never overflows the right edge
      const rawLeft = r.left
      const clampedLeft = Math.min(rawLeft, Math.max(8, window.innerWidth - DROPDOWN_W - 8))

...
function PartialReturnRow({
  ...
  const openDetails = () => navigate(`/deliveries/${delivery._id}`)

  return (
    <div className="overflow-hidden rounded-xl border border-slate-200 bg-white transition-colors hover:bg-amber-50/40">
      {/* Header row — click opens delivery details (same as Delivery Calendar) */}
      <div
        role="link"
        tabIndex={0}
        title={`Open ${delivery.deliveryNo}`}
        className="flex cursor-pointer select-none items-center gap-3 px-4 py-3"
        onClick={openDetails}
        onKeyDown={(e) => {
      ...
      >
        ...
        <div className="min-w-0 flex-1">
          ...
          <div className="text-xs font-semibold text-amber-700">
            {delivery.deliveryNo}
            {delivery.siteName ? <span className="font-normal text-slate-500">{` · ${delivery.siteName}`}</span> : ''}
          </div>
        </div>
        ...
        {/* Chevron — expands line items without opening details */}
        <button
          type="button"
          title={expanded ? 'Hide items' : 'Show items'}
          className="flex h-8 w-8 shrink-0 items-center justify-center rounded-lg border border-slate-200 text-slate-500 hover:bg-white"
          onClick={(e) => {
            e.stopPropagation()
            setExpanded((o) => !o)
          }}
...
export function ReturnDeliveryCalendarPage() {
  ...
  // ── Selected date list (defaults to today — same as Delivery Calendar) ─
  ...
  // ── Load list for selected date (below calendar) ───────────────────────
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    if (next.get('date') !== searchParams.get('date')) {
  ...
  return (
    <div className="w-full font-[inherit]" style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>

      {/* ── Filters — same pattern as Delivery Calendar ── */}
      <div className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
        <div className="px-4 py-3 sm:px-5">
          <div className="mb-2">
            <div className="text-sm font-bold text-slate-900">Filters</div>
            <div className="text-xs text-slate-500">Filter by warehouse &amp; site.</div>
          </div>
          <ReportFiltersBar
            godowns={godowns}
            sites={sites}
      ...
      <div className="flex flex-col gap-5">
        ...
        {/* ── Calendar card — full width, same layout as Delivery Calendar ── */}
        <div className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
          <div className="flex flex-wrap items-center justify-between gap-3 border-b border-slate-100 px-5 py-4">
            <div className="flex items-center gap-3">
              <div className="flex h-9 w-9 items-center justify-center rounded-xl bg-gradient-to-br from-amber-400 to-amber-600">
          ...
          <div className="flex items-center justify-center gap-4 px-5 py-3">
            <button
              type="button"
              onClick={() => setFilters({ month: shiftMonth(month, -1) })}
              className="flex h-8 w-8 items-center justify-center rounded-lg border border-slate-200 text-slate-500 hover:bg-amber-50 hover:border-amber-200 hover:text-amber-700"
            >
              <svg viewBox="0 0 24 24" fill="none" width="14" height="14" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round"><path d="M15 18l-6-6 6-6" /></svg>
            </button>
            <span className="min-w-[120px] text-center text-sm font-semibold text-slate-700">
          ...
          <div className="w-full bg-amber-50/40 px-2 pb-2 sm:px-4">
            <MonthCalendar
              month={month}
              days={calDaysForMonthCalendar}
          ...
          <div className="flex items-center justify-between gap-2 border-t border-slate-100 px-4 py-2.5 text-xs text-slate-400">
            <span>🟡 Tap a date to view returns below; tap a row to open its details.</span>
            <span className="font-medium">Return scheduling overview</span>
          </div>
        </div>

        {/* ── List panel — full width, below the calendar (same as Delivery Calendar) ── */}
        {selectedDate && (
          <div className="flex flex-col gap-4">
            <div className="overflow-hidden rounded-2xl border border-amber-200 bg-gradient-to-r from-amber-50 to-orange-50 px-5 py-4">
              <div className="flex flex-wrap items-start justify-between gap-3">
            ...
            {dailyLoading && (
              <div className="py-12 text-center text-sm text-slate-400">Loading returns…</div>
            )}
            ...
            {!dailyLoading && !dailyError && dailyData && dailyData.length === 0 && (
              <div className="rounded-2xl border border-slate-200 bg-white px-6 py-14 text-center">
                <div className="mb-3 text-4xl">↩</div>
                <p className="font-semibold text-slate-700">No partial returns on this date</p>
                <p className="mt-1 text-sm text-slate-400">
                  No deliveries with outstanding items for this date with the current filters.
                </p>
              </div>
            )}

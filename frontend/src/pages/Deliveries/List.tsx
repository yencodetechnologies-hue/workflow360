...
// // const validTabs: Tab[] = ['all','PROCESSED','PACKED','OUT_FOR_DELIVERY','DELIVERED','RETURN_PICKUP','PENDING_RETURN','COMPLETED','BILLED']
...
//     </span>
//   )}
//   {(d.vehicleLabel || d.returnPickupVehicleLabel) && (d.vehicleType || d.returnPickupVehicleType) ? (
//     <span style={{
//       display: 'inline-block',
//       marginTop: 4,
//       marginLeft: d.selfDelivery ? 6 : 0,
//       padding: '2px 8px',
//       borderRadius: 6,
//       fontSize: 11,
//       fontWeight: 700,
...
function DeliveryProductsPanel({ lines, deliveryAt }: { lines: DeliveryRow['linesSummary']; deliveryAt?: string }) {
  if (!lines?.length) {
    return <div style={{ fontSize: 12, color: '#94a3b8', padding: 12 }}>No products on this delivery.</div>
  }
  const hasReturnActivity = lines.some(
    (l) => (l.returnedQty ?? 0) > 0 || (l.missingQty ?? 0) > 0 || (l.pendingQty ?? 0) > 0,
  )
  const totalPending = lines.reduce((s, l) => s + (l.pendingQty ?? 0), 0)
  const pendingDays = totalPending > 0 ? daysWithClientSince(deliveryAt) : null

  return (
    <div style={{ overflowX: 'auto' }}>
      {pendingDays != null ? (
        <div style={{ padding: '8px 12px 0', fontSize: 12, fontWeight: 600, color: '#b45309' }}>
          {totalPending} qty still with client — {pendingDays} day{pendingDays === 1 ? '' : 's'} since delivery
        </div>
      ) : null}
      <table style={{ width: '100%', borderCollapse: 'collapse', minWidth: 480 }}>
        <thead>
          <tr>
            {['Product', 'Godown', hasReturnActivity ? 'Delivered' : 'Qty', ...(hasReturnActivity ? ['Collected', 'Missing', 'Pending'] : [])].map((h) => (
              <th
                key={h}
                style={{
                  padding: '8px 12px',
                  ...
                  textTransform: 'uppercase',
                  letterSpacing: '0.06em',
                  textAlign: h === 'Product' || h === 'Godown' ? 'left' : 'right',
                  borderBottom: '1px solid #bbf7d0',
                }}
              >
                {h}
        ...
        <tbody>
          {lines.map((line) => (
            <tr key={line.productId}>
              ...
              <td style={{ padding: '8px 12px', fontSize: 13, color: '#64748b' }}>{line.godownName || '—'}</td>
              <td style={{ padding: '8px 12px', fontSize: 13, fontWeight: 600, color: '#0f172a', textAlign: 'right' }}>
                {hasReturnActivity ? (line.dispatchedQty || line.qty) : line.qty}
              </td>
              {hasReturnActivity ? (
                <>
                  <td style={{ padding: '8px 12px', fontSize: 13, fontWeight: 600, color: '#059669', textAlign: 'right' }}>
                  ...
                  <td style={{ padding: '8px 12px', fontSize: 13, fontWeight: 600, color: (line.missingQty ?? 0) > 0 ? '#dc2626' : '#94a3b8', textAlign: 'right' }}>
                    {line.missingQty || 0}
                  </td>
                  <td style={{ padding: '8px 12px', fontSize: 13, fontWeight: 600, color: (line.pendingQty ?? 0) > 0 ? '#b45309' : '#94a3b8', textAlign: 'right' }}>
                    {line.pendingQty || 0}
                  </td>
                </>
              ) : null}
...
const validTabs: Tab[] = ['all','PROCESSED','PACKED','OUT_FOR_DELIVERY','DELIVERED','RETURN_PICKUP','PENDING_RETURN','COMPLETED','BILLED']
  ...
  {(d.vehicleLabel || d.returnPickupVehicleLabel) && (d.vehicleType || d.returnPickupVehicleType) ? (
    <span style={{
      ...
      {(d.returnPickupVehicleType || d.vehicleType) === 'PORTER' ? 'Porter' : (d.returnPickupVehicleType || d.vehicleType) === 'OWN' ? 'Own' : 'Private'}
    </span>
  ) : null}
</td>
                      ...
                      {expandedId === d.id && hasProducts ? (
                        <tr>
                          <td colSpan={isGodownUser ? 7 : 8} style={{ padding: '0 16px 14px', background: rowBg }}>
                            ...
                            >
                              <DeliveryProductsPanel lines={d.linesSummary} deliveryAt={d.deliveryAt} />
                            </div>
                          </td>
                        </tr>
                      ) : null}

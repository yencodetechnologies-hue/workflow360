    ...
    >
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4" style={{ gap: 12 }}>
        {loading ? (
          ...
          <>
            <ClickableCard onClick={() => nav(`/calendar?date=${todayStr}&phase=return`)}>
              <StatCard
                tone="neutral"
            ...
            <ClickableCard onClick={() => nav(`/calendar?date=${todayStr}&phase=return&status=active`)}>
              <StatCard
                tone="good"
            ...
            <ClickableCard onClick={() => nav(`/calendar?date=${todayStr}&phase=return&status=pending`)}>
              <StatCard
                tone="warn"
                label="Returns pending"
                value={String(returnKpis.returnsPending)}
                delta="Awaiting pickup / return"
                icon={<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#d97706" strokeWidth="2"><path d="M3 9l9-7 9 7v11a2 2 0 01-2 2H5a2 2 0 01-2-2z" /><polyline points="9 22 9 12 15 12 15 22" /></svg>}
              />
            </ClickableCard>

            <ClickableCard onClick={() => nav(`/calendar?date=${todayStr}&phase=return&status=completed`)}>
              <StatCard
                tone="good"

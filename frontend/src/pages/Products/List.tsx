  ...
  return (
    ...
    <div style={{ fontFamily: 'inherit', display: 'flex', flexDirection: 'column', gap: 20 }}>
      ...
      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', flexWrap: 'wrap', gap: 12 }}>
        ...
        <button
          onClick={() => {
            ...
            const maxSNo = products.reduce((max, p) => {
              const n = parseInt(p.sNo, 10)
              return isNaN(n) ? max : Math.max(max, n)
            }, 0)
            const nextSNo = String(maxSNo + 1)
            setForm({ name: '', sku: `SKU-${nextSNo}`, category: '', specification: '', rate: '', sNo: nextSNo, imagePath: '' })
            setOpen(true)
          }}
          style={{
            display: 'inline-flex', alignItems: 'center', gap: 6,
      ...
      >
        {saveError && <div className="mb-4 rounded-xl border border-rose-100 bg-rose-50 px-4 py-2.5 text-[13px] text-rose-700">{saveError}</div>}
        <div className="grid grid-cols-1 gap-4">
          <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '8px 12px', borderRadius: 10, background: '#f0fdf4', border: '1px solid #a7f3d0' }}>
            <span style={{ fontSize: 12, fontWeight: 700, color: '#059669' }}>S.No:</span>
            <span style={{ fontSize: 13, fontWeight: 700, color: '#065f46' }}>{form.sNo || '—'}</span>
            {!editingId && <span style={{ fontSize: 11, color: '#6ee7b7', marginLeft: 4 }}>(auto-generated)</span>}
            {editingId && <Input label="" value={form.sNo} onChange={(e) => setForm((f) => ({ ...f, sNo: e.target.value }))} placeholder="e.g. 1" />}
          </div>
          <Input label="Particulars" value={form.name} onChange={(e) => setForm((f) => ({ ...f, name: e.target.value }))} />
          <Input label="Category" value={form.category} onChange={(e) => setForm((f) => ({ ...f, category: e.target.value }))} />
          <Input label="Specification" value={form.specification} onChange={(e) => setForm((f) => ({ ...f, specification: e.target.value }))} />

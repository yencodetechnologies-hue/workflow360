...
type CatalogRow = { productId: string; enabled: boolean; particulars?: string; sku?: string; category?: string; rate?: string }
type StockAgg = { productId: string; qty: number }
type DeliveryRow = { id: string; deliveryNo: string; customerName: string; siteName?: string; status: string; deliveryAt: string; billerPendingReturnAt?: string; billerPendingReturnSlot?: 'MORNING' | 'AFTERNOON' | 'EVENING' }
type DeliveryLineRow = { productId: string; godownId?: string; qty: number }
type DeliveryDetailRow = DeliveryRow & { fromGodownId: string; lines: DeliveryLineRow[] }
type Tab = 'catalog' | 'stock' | 'update'
...
export function GodownsDetailsPage() {
  ...
  // Selected product row for right panel detail
  const [selectedProductId, setSelectedProductId] = useState<string | null>(null)
  // Drill-down modal for "Out of delivery" / "Missing" counts
  const [breakdown, setBreakdown] = useState<{
    productName: string
    kind: 'out' | 'missing'
    rows: { delivery: DeliveryDetailRow; qty: number }[]
  } | null>(null)
  ...
  const deliveryStatsByProduct = useMemo(() => {
    const stats = new Map<string, {
      ...
      missing: number
      outRows: { delivery: DeliveryDetailRow; qty: number }[]
      missingRows: { delivery: DeliveryDetailRow; qty: number }[]
    }>()
    const activeStatuses = ['PROCESSED', 'PACKED', 'OUT_FOR_DELIVERY', 'DISPATCHED', 'DELIVERED', 'PENDING_RETURN']
    const missingStatuses = ['PENDING_RETURN', 'DELIVERED']
    for (const d of deliveries) {
      ...
      for (const line of d.lines) {
        if (!line.productId) continue
        const existing = stats.get(line.productId) ?? { outOfDelivery: 0, missing: 0, outRows: [], missingRows: [] }
        if (activeStatuses.includes(d.status)) {
          existing.outOfDelivery += line.qty
          existing.outRows.push({ delivery: d, qty: line.qty })
        }
        if (missingStatuses.includes(d.status)) {
          existing.missing += line.qty
          existing.missingRows.push({ delivery: d, qty: line.qty })
        }
        stats.set(line.productId, existing)
      }
    }
  ...
  const stockTableRows = useMemo(() => stockRows.map(s => {
    const p = catalogById.get(s.productId)
    const dStats = deliveryStatsByProduct.get(s.productId) ?? { outOfDelivery: 0, missing: 0, outRows: [], missingRows: [] }
  ...
  const updateStockRows = useMemo(() => enabledCatalogRows.map(p => {
    ...
    return { ...p, qty: stockQtyByProductId.get(p.productId) ?? 0, outOfDelivery: dStats.outOfDelivery, missing: dStats.missing, outRows: dStats.outRows, missingRows: dStats.missingRows }
  }), [enabledCatalogRows, stockQtyByProductId, deliveryStatsByProduct])
...
<button style={tabBtn('stock')} onClick={() => setTab('stock')}>
  Stock at godown
                      ...
                      {filteredUpdateStockRows.map((p, idx) => {
                        ...
                        return (
                          ...
                          >
                            ...
                            <td style={{ ...TD, textAlign: 'right', fontWeight: 700, color: '#059669' }}>{formatNumber(p.qty)}</td>
                            <td
                              style={{ ...TD, textAlign: 'right', fontWeight: 700, color: '#C2410C', cursor: p.outOfDelivery > 0 ? 'pointer' : 'default', textDecoration: p.outOfDelivery > 0 ? 'underline' : 'none' }}
                              onClick={e => { e.stopPropagation(); if (p.outOfDelivery > 0) { setBreakdown({ productName: p.particulars ?? p.productId, kind: 'out', rows: p.outRows }); setBreakdownPage(1) } }}
                            >{p.outOfDelivery}</td>
                            <td
                              style={{ ...TD, textAlign: 'right', fontWeight: 700, color: '#DC2626', cursor: p.missing > 0 ? 'pointer' : 'default', textDecoration: p.missing > 0 ? 'underline' : 'none' }}
                              onClick={e => { e.stopPropagation(); if (p.missing > 0) { setBreakdown({ productName: p.particulars ?? p.productId, kind: 'missing', rows: p.missingRows }); setBreakdownPage(1) } }}
                            >{p.missing}</td>
                            <td style={{ ...TD, textAlign: 'right' }} onClick={e => e.stopPropagation()}>
                              <div style={{ display: 'flex', gap: 6, alignItems: 'center', justifyContent: 'flex-end' }}>
                                <input placeholder="+/-" value={adjustDeltaByProduct[p.productId] ?? ''} onChange={e => setAdjustDeltaByProduct(prev => ({ ...prev, [p.productId]: e.target.value }))}
                ...
                <div style={{ flex: 1, overflowY: 'auto', padding: '14px 18px', display: 'flex', flexDirection: 'column', gap: 10 }}>
                  {filteredStockTableRows.map(p => (
                    ...
                    >
                      ...
                      {/* Out of delivery badge */}
                      <div
                        style={{ textAlign: 'center', flexShrink: 0, minWidth: 56, cursor: p.outOfDelivery > 0 ? 'pointer' : 'default' }}
                        onClick={e => { e.stopPropagation(); if (p.outOfDelivery > 0) { setBreakdown({ productName: p.name, kind: 'out', rows: p.outRows }); setBreakdownPage(1) } }}
                      >
                        <div style={{ fontSize: 15, fontWeight: 700, color: '#C2410C', textDecoration: p.outOfDelivery > 0 ? 'underline' : 'none' }}>{p.outOfDelivery}</div>
                        <div style={{ fontSize: 9, color: '#9A3412', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.04em' }}>Out</div>
                      </div>
                      {/* Missing badge */}
                      <div
                        style={{ textAlign: 'center', flexShrink: 0, minWidth: 56, cursor: p.missing > 0 ? 'pointer' : 'default' }}
                        onClick={e => { e.stopPropagation(); if (p.missing > 0) { setBreakdown({ productName: p.name, kind: 'missing', rows: p.missingRows }); setBreakdownPage(1) } }}
                      >
                        <div style={{ fontSize: 15, fontWeight: 700, color: '#DC2626', textDecoration: p.missing > 0 ? 'underline' : 'none' }}>{p.missing}</div>
                        <div style={{ fontSize: 9, color: '#991B1B', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.04em' }}>Missing</div>
                      </div>
                    </div>
                  ))}
      ...
      {/* OUT-OF-DELIVERY / MISSING BREAKDOWN MODAL */}
      <Modal
        open={!!breakdown}
        title={breakdown ? `${breakdown.kind === 'out' ? 'Out of delivery' : 'Missing'} — ${breakdown.productName}` : ''}
        onClose={() => { setBreakdown(null); setBreakdownPage(1) }}
      >
        {breakdown ? (() => {
          const pageSize = 10
          const totalPages = Math.max(1, Math.ceil(breakdown.rows.length / pageSize))
          const page = Math.min(breakdownPage, totalPages)
          const pageRows = breakdown.rows.slice((page - 1) * pageSize, page * pageSize)
          return (
            <div>
              <p style={{ fontSize: 12.5, color: '#7C7A9A', marginBottom: 12 }}>
                {breakdown.kind === 'out'
                  ? 'Deliveries currently holding this product out of the godown.'
                  : 'Deliveries where this product has not yet been confirmed returned.'}
              </p>
              <div style={{ border: '1px solid rgba(83,74,183,0.12)', borderRadius: 10, overflow: 'hidden' }}>
                <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                  <thead>
                    <tr>
                      <th style={TH}>Delivery</th>
                      <th style={TH}>Customer</th>
                      {breakdown.kind === 'out' ? <th style={TH}>Delivery date</th> : null}
                      <th style={{ ...TH, textAlign: 'right' }}>Qty</th>
                    </tr>
                  </thead>
                  <tbody>
                    {pageRows.map(({ delivery, qty }, i) => {
                      const dateLabel = delivery.deliveryAt ? formatDateTime(delivery.deliveryAt) : '—'
                      return (
                        <tr
                          key={`${delivery.id}-${i}`}
                        ...
                        >
                          <td style={TD}>
                            <span style={{ color: '#059669', fontWeight: 600 }}>{delivery.deliveryNo}</span>
                          </td>
                          <td style={TD}>{delivery.customerName || delivery.siteName || '—'}</td>
                          {breakdown.kind === 'out' ? <td style={{ ...TD, whiteSpace: 'nowrap' }}>{dateLabel}</td> : null}
              ...
              {breakdown.rows.length === 0 ? (
                <p style={{ padding: '18px 4px', textAlign: 'center', color: '#7C7A9A', fontSize: 13 }}>No deliveries to show.</p>
              ) : totalPages > 1 ? (
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginTop: 12 }}>
                  <button
                    disabled={page <= 1}
                    onClick={() => setBreakdownPage(p => Math.max(1, p - 1))}
                    style={{ padding: '6px 12px', borderRadius: 7, border: '1px solid rgba(83,74,183,0.18)', background: '#fff', fontSize: 12, fontWeight: 600, color: '#1E1A4E', cursor: page <= 1 ? 'not-allowed' : 'pointer', opacity: page <= 1 ? 0.4 : 1 }}
                  >
                    ← Prev
                  </button>
                  <span style={{ fontSize: 12, color: '#7C7A9A', fontWeight: 600 }}>
                    Page {page} of {totalPages} · {breakdown.rows.length} total
                  </span>
                  <button
                    disabled={page >= totalPages}
                    onClick={() => setBreakdownPage(p => Math.min(totalPages, p + 1))}
                    style={{ padding: '6px 12px', borderRadius: 7, border: '1px solid rgba(83,74,183,0.18)', background: '#fff', fontSize: 12, fontWeight: 600, color: '#1E1A4E', cursor: page >= totalPages ? 'not-allowed' : 'pointer', opacity: page >= totalPages ? 0.4 : 1 }}
                  >
                    Next →
                  </button>
                </div>

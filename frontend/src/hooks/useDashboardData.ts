...
type StatusCountsResult = {
  total: number
  byStatus: Record<string, number>
}

// Build KPIs from the all-time status counts (not limited to today's date)
function mapStatusCounts(counts: StatusCountsResult | null, damaged: number, lost: number): TodayKpis {
...
// Build today's KPIs from the daily report (for trend bar labels/counts only)
function mapDailyTodayCounts(report: DailyReport | null) {
  const s = report?.summary
  return { total: s?.total ?? 0, damaged: s?.damaged ?? 0, lost: s?.lost ?? 0 }
}
...
export function useDashboardData() {
  ...
  const [error, setError] = useState<string | null>(null)
  const [todayReport, setTodayReport] = useState<DailyReport | null>(null)
  const [statusCountsData, setStatusCountsData] = useState<StatusCountsResult | null>(null)
  const [trend, setTrend] = useState<TrendDay[]>([])
  const [godownsWithStock, setGodownsWithStock] = useState<GodownWithStock[]>([])
  const [recentActivity, setRecentActivity] = useState<DeliveryActivity[]>([])
  const [missing, setMissing] = useState<MissingRow[]>([])
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      const [dailyResults, statusCounts, missingRows, godownRows, stockRows, deliveryRows] = await Promise.all([
        // 7-day trend
        Promise.all(
          dates.map((date) =>
            apiFetch<DailyReport>(`/reports/daily?date=${encodeURIComponent(date)}${godownQ}`, { token }),
        ...
        // All-time status counts for KPI cards
        apiFetch<StatusCountsResult>(`/reports/status-counts`, { token }).catch(() => null),
        apiFetch<MissingRow[]>(`/reports/missing?limit=100${godownQ}`, { token }).catch(() => [] as MissingRow[]),
        apiFetch<GodownRow[]>('/godowns', { token }),
        apiFetch<Array<{ godownId: string; productId: string; qty: number }>>(
          `/reports/stock${godownId ? `?godownId=${encodeURIComponent(godownId)}` : ''}`,
      ...
      const todayData = dailyResults[dailyResults.length - 1] ?? null
      setTodayReport(todayData)
      setStatusCountsData(statusCounts)
  ...
  const kpis = useMemo(() => {
    const todayCounts = mapDailyTodayCounts(todayReport)
    const k = mapStatusCounts(statusCountsData, todayCounts.damaged, todayCounts.lost)
    return { ...k, totalStock }
  }, [todayReport, statusCountsData, totalStock])

...
export function DeliveryStatusSelect({
  ...
  const postVehicleTransition = async (
    ...
    try {
      if (kind === 'out') {
        const res = await postDeliveryVehicle(deliveryId, token, vehicleNumber, status, { forAdmin })
        setValue(res.status)
  ...
  return (
    <div className={cn('inline-flex flex-wrap items-center gap-2', className)}>
      ...
      <VehicleNumberModal
        ...
        onConfirm={(v) => postVehicleTransition('out', v)}
      />
      ...
      <ReturnPickupVehicleModal
        ...
        onClose={cancelVehicleModal}
        onConfirm={(v) => postVehicleTransition('return', v)}
      />
    </div>
  )
}

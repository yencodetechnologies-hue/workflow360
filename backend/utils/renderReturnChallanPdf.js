...
function buildReturnLines(delivery) {
  ...
  for (const line of delivery.lines || []) {
    ...
    if (returnedQty > 0) {
      ...
      if (!seen.has(key)) {
        seen.add(key)
        items.push({ ...line, _qtyField: 'returnedQty', _note: 'Collected' })
      }
    }
  }
  ...
  for (const line of delivery.billerPendingReturnLines || []) {
    const key = `pending-${line.productId}`
    if (!seen.has(key) && (line.qty || 0) > 0) {
      seen.add(key)
      items.push({
        productId: line.productId,
 ...
 */
function renderReturnChallanPdf(doc, delivery) {
  const collectedQty = (delivery.lines || []).reduce((s, l) => s + (Number(l.returnedQty) || 0), 0)
  const pendingQty = sumQty(delivery.billerPendingReturnLines)

  const leftRows = [
    ['Challan No', delivery.challanNo || delivery.deliveryNo || '—'],
    ['Biller return', delivery.billerReturnSubmittedAt ? formatDateTime(delivery.billerReturnSubmittedAt) : '—'],
    ['Collected qty', String(collectedQty)],
    ['Pending qty', String(pendingQty)],
  ]

...
//   doc.text('Walker Agencies', textX, MARGIN + 20, { lineBreak: false })
...
function lineFromProduct(line, qtyField = 'qty') {
  const p = line.productId
  const baseName = p?.particulars || p?.name || line.particulars || 'Item'
  return {
    name: line._note ? `${baseName}  (${line._note})` : baseName,
    sku: p?.sku || p?.s_no || line.sku || '—',
    unit: p?.unit || line.unit || '—',
    qty: line[qtyField] ?? line.qty ?? '—',
  }

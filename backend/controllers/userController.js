...
async function createUser(req, res) {
  try {
    const { email, password, role, godownId, active } = req.body || {}
    ...
    return res.status(201).json({ id: String(user._id), email: user.email, role: user.role, godownId: user.godownId, active: user.active })
  } catch (err) {
...
module.exports = { listUsers, createUser, setUserActive, resetPassword }

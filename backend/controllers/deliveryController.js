...
async function assignOutForDeliveryVehicle(delivery, userId, vehicleNumber, driverName, driverPhone) {
...
async function outForDelivery(req, res) {
  try {
    ...
    const { driver } = await assignOutForDeliveryVehicle(delivery, req.user.id, vehicleNumber, driverName, driverPhone)
...
async function assignReturnPickup(req, res) {
  try {
    const { vehicleNumber, returnPickupAt, driverName, driverPhone } = req.body || {}
    if (!vehicleNumber || !String(vehicleNumber).trim()) {
      return res.status(400).json({ message: 'vehicleNumber required' })
    }

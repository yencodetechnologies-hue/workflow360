      ...
      for (const line of billerDamagedLines) {
        if (line.qty > 0) {
          ledgerEntries.push({
            godownId: new mongoose.Types.ObjectId(String(godownId)),
            productId: new mongoose.Types.ObjectId(String(line.productId)),
            qtyDelta: +line.qty,  // restock: physically returned damaged item
            reason: 'RETURN_DAMAGE',
            refType: 'BillerReturn',
            refId: String(delivery._id),
            note: line.note || `Damaged item returned & restocked - ${delivery.deliveryNo}`,
          })
        }
      }
      for (const line of billerMissingLines) {
        if (line.qty > 0) {
          ledgerEntries.push({
            godownId: new mongoose.Types.ObjectId(String(godownId)),
            productId: new mongoose.Types.ObjectId(String(line.productId)),
            qtyDelta: +line.qty,  // restock: physically returned missing item
            reason: 'RETURN_MISSING',
            refType: 'BillerReturn',
            refId: String(delivery._id),
            note: line.note || `Missing item returned & restocked - ${delivery.deliveryNo}`,
          })
        }
      }
      if (ledgerEntries.length > 0) {
